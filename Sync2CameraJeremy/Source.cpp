...
int main(int argc, char** argv)
{
	...
	else if (num_devices == 2)
	{
			...
			greenscreen_duration)
		{
			...
			// Create k4a_image_t
			k4a_image_t firstDepthImage = main_depth_image.handle();
			k4a_image_t secondDepthImage = secondary_depth_image.handle();


			// First point cloud
			k4a_image_t xyTable1 = NULL;
			k4a_image_t pointCloud1 = NULL;

			k4a_result_t result11 = k4a_image_create(main_depth_image.get_format(),
				main_calibration.depth_camera_calibration.resolution_width,
				main_calibration.depth_camera_calibration.resolution_height,
				main_calibration.depth_camera_calibration.resolution_width *
				(int)sizeof(k4a_float2_t),
				&xyTable1);

			create_xy_table(&main_calibration, xyTable1);

			k4a_result_t result12 = k4a_image_create(main_depth_image.get_format(),
				main_calibration.depth_camera_calibration.resolution_width,
				main_calibration.depth_camera_calibration.resolution_height,
				main_calibration.depth_camera_calibration.resolution_width *
			...
			int pointCount = 0;
			generate_point_cloud(firstDepthImage, xyTable1, pointCloud1, &pointCount);
			std::ostringstream outputFileName;
			outputFileName << "main_" << index << ".ply";
			write_point_cloud(outputFileName.str().c_str(), pointCloud1, pointCount);


			// Second point cloud
			k4a_image_t xyTable2 = NULL;
			k4a_image_t pointCloud2 = NULL;

			k4a_result_t result21 = k4a_image_create(K4A_IMAGE_FORMAT_CUSTOM,
				secondary_depth_to_main_color_cal.depth_camera_calibration.resolution_width,
				secondary_depth_to_main_color_cal.depth_camera_calibration.resolution_height,
				secondary_depth_to_main_color_cal.depth_camera_calibration.resolution_width *
				(int)sizeof(k4a_float2_t),
				&xyTable2);
			...
			k4a_result_t result22 = k4a_image_create(K4A_IMAGE_FORMAT_CUSTOM,
				secondary_depth_to_main_color_cal.depth_camera_calibration.resolution_width,
				secondary_depth_to_main_color_cal.depth_camera_calibration.resolution_height,
				secondary_depth_to_main_color_cal.depth_camera_calibration.resolution_width *
				(int)sizeof(k4a_float3_t),
			...
			pointCount = 0;
			generate_point_cloud(firstDepthImage, xyTable1, pointCloud2, &pointCount);
			std::ostringstream outputFileName2;
			outputFileName2 << "sub_" << index << ".ply";
			write_point_cloud(outputFileName2.str().c_str(), pointCloud2, pointCount);
			index++;


			// DU Mod
			// May be able to delete the bottom part since we don't care about the green screen
